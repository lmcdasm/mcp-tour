...
import (
	...
	"mcp-go-client/mcplib/jsonschema"
)
...
// DASM - Added in a generic testPromptHandler  and function to match the needs for NewPrompt
type testPromptHandler[T any] struct{}

func (h testPromptHandler[TReq]) Handle(
	ctx context.Context,
	...
	return &mcp.GetPromptResult{
		Description: "Prompt Stub Description",
		Messages: []*mcp.PromptMessage{
			{
				Role: "assistant",
				Content: &mcp.Content{
					Text: "this is a prompt stubbed message",
				},
			},
		},
	}, nil
...
// DASM - Apple, Banana and Cherry Prompt "types" (used for testing)
...
func (h testPromptHandler[T]) HandlePrompt() {
    // no-op stub
}
...
func TestListPrompts(t *testing.T) {
	promptA := mcp.NewPrompt("apple", "apple prompt", testPromptHandler[AppleReq]{}.Handle)
	promptB := mcp.NewPrompt("banana", "banana prompt", testPromptHandler[BananaReq]{}.Handle)
	promptC := mcp.NewPrompt("cherry", "cherry prompt", testPromptHandler[CherryReq]{}.Handle)
